var Main = (function () {
    ...
    Main.prototype.draw = function () {
        ...
        var svg = d3.select("body").append("svg").attr("width", this.w).attr("height", this.h);
        var rects = svg.selectAll("rect").data(this.dataset).enter().append("rect");
        ...
        rects.on("mouseover", function (data, index) {
            d3.select(this).attr("fill", d3.rgb(color(index)).brighter().toString());
        }).on("mouseout", function (data, index) {
            d3.select(this).attr("fill", d3.rgb(color(index)).toString());
        });
    };
    Main.prototype.createXrange = function () {
        var range = d3.scale.linear();
        range.domain([0, this.numData]).range([this.padding, this.w - this.padding]);
        return range;
    };
    Main.prototype.createYrange = function () {
        var range = d3.scale.linear();
        range.domain([0, this.dataRange]).range([this.padding, this.h - this.padding]);
        return range;
    };
    return Main;
})();
